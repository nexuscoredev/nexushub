...
export type PersonalViewOption = {
  id: DrinkCartaViewMode;
  label: string;
  shortLabel: string;
};

type PersonalViewMenuProps = {
  viewMode: DrinkCartaViewMode;
  options: PersonalViewOption[];
  activeLabel: string;
...
export function PersonalViewMenu({
  viewMode,
  options,
  activeLabel,
  onViewModeChange,
}: PersonalViewMenuProps) {
  ...
  return (
    <div className={styles.viewMenuRoot} ref={rootRef}>
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        aria-label={`Visualização: ${activeLabel}`}
        title={`Visualização: ${activeLabel}`}
      >
