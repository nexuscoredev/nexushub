...
export const DEV_WHITEBOARD_STORAGE_KEY = 'nexushub-dev-whiteboard-embed';

/** URL padrão do quadro NEXUS (opcional — Vercel env). */
export const DEV_WHITEBOARD_DEFAULT =
  (import.meta.env.VITE_NEXUS_WHITEBOARD_EMBED as string | undefined)?.trim() ?? '';

/** Converte link Miro / Web Whiteboard para live-embed (iframe permitido). */
export function toMiroLiveEmbedUrl(input: string): string | null {
  const trimmed = input.trim();
  if (!trimmed) return null;

  try {
    const url = new URL(trimmed);
    if (url.hostname.includes('webwhiteboard.com')) {
      return null;
    }

    if (url.pathname.includes('/live-embed/')) {
    ...
    const boardMatch = url.pathname.match(/\/app\/board\/([^/]+)/i);
    if (boardMatch?.[1]) {
      return `https://miro.com/app/live-embed/${boardMatch[1]}/?autoplay=true`;
    }
  } catch {
    return null;
  }
...
export function loadWhiteboardEmbedUrl(): string {
  if (DEV_WHITEBOARD_DEFAULT) return DEV_WHITEBOARD_DEFAULT;
  try {
    return localStorage.getItem(DEV_WHITEBOARD_STORAGE_KEY)?.trim() ?? '';
  } catch {
    return '';
  }
}

export function saveWhiteboardEmbedUrl(embedUrl: string) {
  localStorage.setItem(DEV_WHITEBOARD_STORAGE_KEY, embedUrl.trim());
}

export function miroBoardViewUrl(embedUrl: string): string {
  const match = embedUrl.match(/\/live-embed\/([^/?]+)/i);
  if (match?.[1]) return `https://miro.com/app/board/${match[1]}/`;
  return embedUrl;
}
